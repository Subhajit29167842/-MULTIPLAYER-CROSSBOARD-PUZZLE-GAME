...
/*The Server class constructor takes an integer parameter port, which represents the port number on which the server will listen for client connections.
Inside the constructor, the serverPort member variable is initialized with the provided port value.*/
    void start() {
...
The start() method is responsible for initializing the server socket, binding it to the specified port, and putting the server in a listening state.

It starts by creating a TCP socket using the socket() function.
If the socket creation fails, it prints an error message and returns.
It then defines the server address structure, specifying the address family (AF_INET for IPv4), the IP address (INADDR_ANY to bind to all available interfaces), and the port number (serverPort).
It calls the bind() function to associate the socket with the specified address.
If the binding fails, it prints an error message and returns.
It calls the listen() function to put the socket in a listening state, allowing it to accept incoming client connections.
It then enters an infinite loop, continuously accepting new client connections*/
